...
curl -s "https://api.tzkt.io/v1/operations/delegations?status.eq=$dstatus&limit=$limit&offset.pg=$offset&timestamp.ge=$start&timestamp.lt=$end" | jq '. | length'
...
###
### Off course every offset is not a PAGE ;)
...
curl -s "https://api.tzkt.io/v1/operations/delegations?status.eq=$dstatus&limit=$limit&offset.pg=$offset&timestamp.ge=$start&timestamp.lt=$end" | jq > $fname
